...
<body class="bg-dark" data-bs-scrollspy="scroll" data-bs-target=".navbar">
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active text-white" href="huawei.php"><i class="fa-solid fa-mobile-screen"></i> Huawei Cases</a>
                    </li>
                    <li class="nav-item">
    ...
    <div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel">
        <div class="carousel-inner">
            <div class="carousel-item active">
                <img src="image/hero_huawei.png" class="car d-block w-100" alt="hero2">
    ...
    <!-- Main Content -->
    <div class="container">
        <h2 class="text-white text-center mt-4">Huawei Phone Cases</h2>

        <!-- Huawei Section -->

        <!-- Php Code for dispalying product images from database -->
        <?php
        $query = "SELECT * FROM HUAWEI ORDER BY HUAWEI_ID ASC";
        $result = mysqli_query($connect, $query);
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_array($result)) {
        ?>
                <div class="col-lg-3 col-md-6 float-start">
                    <form method="post" action="huawei.php?action=add&HUAWEI_ID=<?php echo $row["HUAWEI_ID"]; ?>">
                        ...
                        <div class="row justify-content-center ">
                            <div class="card-banner m-3">
                                <div class="card  shadow m-4">
                                    <div class="inner ">
                                        <img src="image/<?php echo $row["HUAWEI_IMAGE"]; ?>" class="card-img-top" />
                                    </div>
                                    <div class="card-body text-center ">
                                        <h5 class="text-dark"><?php echo $row["HUAWEI_TYPE"]; ?></i>
                                        </h5>
                                        <input type="text" name="quantity" value="1" class="form-control" />
                                        <p class="text-danger">€ <?php echo $row["HUAWEI_PRICE"]; ?></p>
                                        <input type="hidden" name="hidden_name" value="<?php echo $row["HUAWEI_TYPE"]; ?>" />
                                        <input type="hidden" name="hidden_price" value="<?php echo $row["HUAWEI_PRICE"]; ?>" />
                                        <button type="button" class="btn btn-dark text-danger" style="pointer-events: none;">Out of stock</button>
                                    </div>
                                </div>
                            </div>
                        </div>
